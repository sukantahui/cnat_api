...
class StoreStudentRequestBasic extends BaseRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        // ✅ Set default district_id if not provided
        if (!$this->has('district_id') || empty($this->district_id)) {
            $this->merge([
                'district_id' => 2
            ]);
        }
        ...
        // ✅ Default gender_id
        if (!$this->has('gender_id') || empty($this->gender_id)) {
            $this->merge([
                'gender_id' => 1
            ]);
        }
